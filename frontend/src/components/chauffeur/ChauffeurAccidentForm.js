...
function ChauffeurAccidentForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const accidentData = {
        // ✅ Noms exacts des champs du DTO backend AccidentDTO
        dateAccident:        formData.date,
        lieuAccident:        formData.lieu,         // ← était "lieu", doit être "lieuAccident"
        description:         formData.description,
        personnesImpliquees: formData.temoins,       // ← était "temoins", doit être "personnesImpliquees"
        degats:              formData.degats,
        chauffeurId:         user.id,
        missionId:           formData.missionId ? parseInt(formData.missionId) : null,
        statut:              'DÉCLARÉ'
        // vehiculeId intentionnellement absent : le chauffeur n'a pas forcément un véhicule assigné
      };

      await createAccident(accidentData);
      showSuccess('Accident déclaré avec succès. Le chef de parc a été notifié.');
  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit} className="user-form">
        ...
        <div className="form-row">
          <div className="form-group">
            <label htmlFor="temoins">Témoins / Personnes impliquées</label>
            <input
              type="text"
              id="temoins"
              name="temoins"
